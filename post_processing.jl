...
function discretize_for_mdp_course(root_node::NODE)
    q = Queue{NODE}()
    enqueue!(q, root_node)
    while length(q) > 0
        curr_node = dequeue!(q)
        next_nodes = length(curr_node.children) > 0 ? curr_node.children : discretize_course!(curr_node)
        for i = 1:length(next_nodes)
            enqueue!(q, next_nodes[i])
        end 
    end
...
function discretize_course!(curr_node::NODE)
    ubs = unnormalize_bounds(curr_node.ubs)
    lbs = unnormalize_bounds(curr_node.lbs)

    dims_to_split = []
    curr_diff = ubs .- lbs
    curr_diff[1] > course_hwidth ? push!(dims_to_split, 1) : nothing
    curr_diff[2] > course_ḣ₀width ? push!(dims_to_split, 2) : nothing
